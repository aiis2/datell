...
assert.match(readme, /single installable skill|merged skill|prefer MCP/i, 'README.md should explain the unified install surface');
...
assert.equal(marketplace.plugins.length, 1, 'marketplace.json should define one merged installable skill');
assert.deepEqual(
  marketplace.plugins.map((plugin) => plugin.name).sort(),
  ['datell-visual-report-preview'],
  'marketplace.json should expose only the merged visual report skill name'
);
assert.deepEqual(
  marketplace.plugins.map((plugin) => plugin.skills[0]).sort(),
  ['./skills/datell-visual-report-preview'],
  'marketplace.json should point to the merged skill directory'
);
...
assert.match(skillMarkdown, /single installable skill|merged/i, 'SKILL.md should document the unified install surface');
