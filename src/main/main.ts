 ...
 * THEME-HDR FIX: Accept palette to override --bg-header with the active palette primary color.
 */
 ...
 * THEME-HDR FIX: Pass palette through so header background follows the active palette.
 */
function buildPrintModeHtml(html: string, themeId = 'business', layoutId = 'default', palette?: ExportPalette): string {
  let result = injectVendorLibs(html, themeId, layoutId, palette);
...
ipcMain.handle('save-pdf', async (_event, htmlOrArgs: string | { html: string; title: string; themeId?: string; layoutId?: string; palette?: ExportPalette }, titleArg?: string) => {
...
// EXP-01 FIX: Accept { html, title, themeId, layoutId } object so renderer can pass current theme
ipcMain.handle('capture-report', async (_event, htmlOrArgs: string | { html: string; title: string; themeId?: string; layoutId?: string; palette?: ExportPalette }, titleArg?: string) => {
  const html     = typeof htmlOrArgs === 'string' ? htmlOrArgs : htmlOrArgs.html;
  const title    = typeof htmlOrArgs === 'string' ? (titleArg ?? '') : htmlOrArgs.title;
  const themeId  = typeof htmlOrArgs === 'object' ? (htmlOrArgs.themeId  ?? 'business') : 'business';
  const layoutId = typeof htmlOrArgs === 'object' ? (htmlOrArgs.layoutId ?? 'default')  : 'default';
  ...
  //   cards, header and layout all preserve the active theme.
  {
    ...
    if (headInject) {
      if (outputHtml.includes('</head>')) {
        ...
        outputHtml = outputHtml.replace('<body', () => `${headInject}\n<body`);
      } else {
        outputHtml = headInject + '\n' + outputHtml;
      }
    }
  }
...
// Returns available=false immediately in community/OSS builds (ENTERPRISE_BUILD=false).
ipcMain.handle('enterprise:pluginStatus', () => {
  if (!ENTERPRISE_BUILD) return { available: false, meta: null };
  return {
...
app.whenReady().then(async () => {
  // Load enterprise model plugin only in enterprise builds.
  // Gracefully absent in community/OSS mode — no plugin file is required.
  if (ENTERPRISE_BUILD) {
