...
<div class="report-zones">
  ...
  <!-- ② 兼容模式筛选控件行（可选；新报表优先使用 data-filter-id + data-filter-type 标准控件） -->
...
filter-controls.js 只负责把控件变化转换成统一的 filterChange 事件，不会自动让图表重绘。
如果页面里出现任何筛选控件，必须二选一：
1. 目标卡片提供 data-sql + __report_data_context__，交给 InteractivityEngine 自动刷新。
2. 每个受影响图表或 KPI 卡脚本监听 filterChange，并从 window.__FILTER_STATE__ 读取完整筛选状态后重绘或更新 DOM。
禁止只输出 zone-filter / filter-zone-select / data-filter-field 这类筛选 UI 而不提供上述任一路径。
只要页面出现筛选控件，受影响的 KPI/图表/表格都必须有更新路径；禁止出现“图表或表格会动，但顶部摘要 KPI 保持初始静态值”的半联动页面。
如果某个 KPI 卡片是固定目标、阈值或说明信息，明确不应该跟随筛选变化，必须在最外层 card 显式写 data-kpi-static="true" 作为静态豁免标记；未标记时默认视为动态 KPI。
为避免生成期校验误判，手写 KPI 联动时请在同一个 <script> 块内同时出现 filterChange/cardUpdate 监听、对应 data-card-id 选择器，以及实际 DOM 更新语句（如 textContent / innerHTML / .text() / .html()）。
...
zone-filter / data-filter-field 仅为兼容旧报表的 legacy 协议，新代码优先使用本节的标准控件写法。
只要页面出现筛选控件，受影响的 KPI/图表/表格都必须有更新路径：要么使用 data-sql 驱动刷新，要么显式监听 filterChange + __FILTER_STATE__ / cardUpdate 更新 DOM 或重绘图表；否则该 HTML 视为无效，禁止输出。
若某个 KPI 卡片不参与筛选联动，必须显式添加 data-kpi-static="true"，否则默认按动态 KPI 校验。
手写 KPI 联动请保持“监听器 + data-card-id 引用 + DOM 更新语句”位于同一个 <script> 块，避免生成期校验无法识别。
...
- **如果数据存在层级维度（如 区域→门店→经办人、年→月→日、类目→品牌→SKU），或用户明确要求上下钻，必须输出至少一个完整 drill_down 协议**：同一卡片上同时给出 \`action:"drill_down"\`、\`drillSql\`、\`drillDimension\`，并在 \`__report_data_context__\` 里提供匹配 cardId 的 \`drillPaths\`。
- **禁止只写“支持下钻/点击查看明细”这类说明文字而不输出协议**。没有 \`drill_down + drillSql + drillDimension + drillPaths\` 的报表视为不合格。
