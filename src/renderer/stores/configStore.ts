...
const LOCKED_PRO_MODEL: ModelConfig = {
  ...
  baseUrl: 'https://srv.pajk-ent.com/ai-link/v1/',
  locked: true,
  lockedTier: 'pro',
};
...
export const useConfigStore = create<ConfigState>((set, get) => {
  ...
  const initialModels = [proModelInstance, basicModelInstance, ...(userModels.length ? userModels : DEFAULT_MODELS)];
  const initialActiveModelId = initialModels.length > 0
    ? (initialModels.some((m) => m.id === saved.activeModelId)
  ...
  return {
    ...
    setEnterprisePluginAvailable: (available) => { set({ enterprisePluginAvailable: available }); },
    ...
    hydrate: () => {
      ...
      if (saved.models) {
        ...
        const activeModelId = models.some((m) => m.id === saved.activeModelId)
          ? (saved.activeModelId as string)
          : (models[0]?.id || '');
